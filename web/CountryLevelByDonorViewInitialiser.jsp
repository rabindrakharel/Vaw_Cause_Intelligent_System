...
cityCircleD1= new google.maps.Circle({
        ...
        fillOpacity: 0.5,
        map: map,
        center: new google.maps.LatLng(<%=SmallDest.get(0) %>,<%=SmallDest.get(1) %>),
        radius: 100000
      });
  <% 
iter=0; 
...
{iter++; %> 
      ...
      cityCircleR<%=iter%> = new google.maps.Circle({
...
<%}%>
  ...
  animateCircle();  	
   ...
   google.maps.event.addListener(cityCircleD1, 'click',function open_chart_popup() {
   window.open('highchart.html?id='+cityCircleD1.getCenter().lat()+'b='+cityCircleR1.getCenter().lng(),'chart popup title', 'width=600px height=400px');   
    });
